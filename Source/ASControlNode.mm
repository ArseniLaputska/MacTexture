...
#define kASControlNodeExpandedInset (-70.0f)
...
- (void)touchesBegan:(NSSet *)touches withEvent:(NSEvent *)event
{
  ...
  if (touches.count > 1 || self.tracking) {
    ...
    ASControlNodeEvent controlEventMask = ASControlNodeEventTouchDown;
...
- (BOOL)gestureRecognizerShouldBegin:(NSGestureRecognizer *)gestureRecognizer
{
  ...
  if (self.enabled && [gestureRecognizer isKindOfClass:[NSClickGestureRecognizer class]] && gestureRecognizer.view != self.view) {
    NSClickGestureRecognizer *tapRecognizer = (NSClickGestureRecognizer *)gestureRecognizer;
  ...
  if (!_controlEventDispatchTable) {
    ...
    if ([ASControlNode enableHitTestDebug] && _debugHighlightOverlay == nil) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        // add a highlight overlay node with area of ASControlNode + NSEdgeInsets
...
- (void)sendActionsForControlEvents:(ASControlNodeEvent)controlEvents withEvent:(NSEvent *)event
{
      ...
      (ASControlNodeEvent controlEvent)
      {
        ...
        for (ASControlTargetAction *targetAction in self->_controlEventDispatchTable[_ASControlNodeEventKeyForControlEvent(controlEvent)]) {
          ...
          if (!resolvedTargetAction.target && targetAction.createdWithNoTarget) {
            ...
            resolvedTargetAction.target = [self.view supplementalTargetForAction:resolvedTargetAction.action sender:self];
          }
...
CGRect _ASControlNodeGetExpandedBounds(ASControlNode *controlNode) {
  return CGRectInset(NSEdgeInsetsInsetRect(controlNode.view.bounds, controlNode.hitTestSlop), kASControlNodeExpandedInset, kASControlNodeExpandedInset);
}
...
- (BOOL)beginTrackingWithTouch:(NSTouch *)touch withEvent:(NSEvent *)touchEvent
{
  return YES;
}

- (BOOL)continueTrackingWithTouch:(NSTouch *)touch withEvent:(NSEvent *)touchEvent
{
  return YES;
}

- (void)cancelTrackingWithEvent:(NSEvent *)touchEvent
{
  // Subclass hook
}

- (void)endTrackingWithTouch:(NSTouch *)touch withEvent:(NSEvent *)touchEvent
{
  // Subclass hook
}
