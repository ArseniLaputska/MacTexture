  ...
  NSMutableArray<ASDisplayNode *> *children = [[NSMutableArray alloc] initWithCapacity:2];
  {
    ...
    if (children.count == 2) {
      ...
      if (_laysOutHorizontally) {
        firstChildStyle.margin = ASEdgeInsetsMake(NSEdgeInsetsMake(0, 0, 0, _contentSpacing));
      } else {
        firstChildStyle.margin = ASEdgeInsetsMake(NSEdgeInsetsMake(0, 0, _contentSpacing, 0));
      }
    }

    // Add padding to button
    if (NSEdgeInsetsEqualToEdgeInsets(NSEdgeInsetsZero, _contentEdgeInsets) == NO) {
    ...
    if (_backgroundImageNode.image) {
      ...
      _backgroundImageNode.style.position = ASEdgeInsetsMake(NSEdgeInsetsZero);
    }
  }
