...
- (instancetype)initWithStyle:(NSTableViewStyle)style
{
  if (self = [super init]) {
...
- (instancetype)init
{
  return [self initWithStyle:NSTableViewStylePlain];
}
...
- (BOOL)allowsSelectionDuringEditing
{
  if ([self pendingState]) {
    ...
    return /*self.view.allowsSelectionDuringEditing*/false;
  }
}
...
- (BOOL)allowsMultipleSelectionDuringEditing
{
  if ([self pendingState]) {
    ...
    return /*self.view.allowsMultipleSelectionDuringEditing*/false;
  }
}
...
- (void)selectRowAtIndexPath:(nullable NSIndexPath *)indexPath animated:(BOOL)animated
{
  ...
  if (indexPath != nil) {
    [tableView selectRow:indexPath.item byExtendingSelection:false];
  } else {
...
- (CGRect)rectForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  indexPath = [tableView convertIndexPathFromTableNode:indexPath waitingIfNeeded:YES];
  return [tableView rectOfRow:indexPath.item];
}

- (nullable __kindof NSTableCellView *)cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ASDisplayNodeAssertMainThread();
  ASTableView *tableView = self.view;
