...
({                                                                                            \
  NSCollectionViewFlowLayout *flowLayout = ASDynamicCast(layout, NSCollectionViewFlowLayout); \
...
static const NSUInteger kASCollectionViewAnimationNone = NSTableViewAnimationEffectNone;
...
@interface ASCollectionView () <ASRangeControllerDataSource, ASRangeControllerDelegate, ASDataControllerSource, ASCellNodeInteractionDelegate, ASDelegateProxyInterceptor, ASBatchFetchingScrollView, ASCALayerExtendedDelegate, NSCollectionViewDelegateFlowLayout> {
...
- (instancetype)initWithCollectionViewLayout:(NSCollectionViewLayout *)layout
{
...
- (instancetype)_initWithFrame:(CGRect)frame collectionViewLayout:(NSCollectionViewLayout *)layout layoutFacilitator:(id<ASCollectionViewLayoutFacilitatorProtocol>)layoutFacilitator owningNode:(ASCollectionNode *)owningNode
{
...
//  super.prefetchingEnabled = NO;
  ...
  super.delegate = (id<NSCollectionViewDelegate>)_proxyDelegate;
  
  _proxyDataSource = [[ASCollectionViewProxy alloc] initWithTarget:nil interceptor:self];
  super.dataSource = (id<NSCollectionViewDataSource>)_proxyDataSource;
  ...
  [self registerClass:[_ASCollectionViewCell class] forItemWithIdentifier:kReuseIdentifier];
...
- (void)scrollToItemAtIndexPath:(NSIndexPath *)indexPath atScrollPosition:(NSCollectionViewScrollPosition)scrollPosition animated:(BOOL)animated
{
  if ([self validateIndexPath:indexPath]) {
    NSSet *setOfIndexes = [NSSet setWithObject:indexPath];
    [super scrollToItemsAtIndexPaths:setOfIndexes scrollPosition:scrollPosition];
  }
}
...
- (void)setCollectionViewLayout:(NSCollectionViewLayout *)collectionViewLayout
{
  ASDisplayNodeAssertMainThread();
...
- (nullable NSString *)modelIdentifierForElementAtIndexPath:(NSIndexPath *)indexPath inView:(NSView *)view {
...
- (nullable NSIndexPath *)indexPathForElementWithModelIdentifier:(NSString *)identifier inView:(NSView *)view {
    if (_asyncDataSourceFlags.modelIdentifierMethods) {
        GET_COLLECTIONNODE_OR_RETURN(collectionNode, nil);
...
- (void)_configureCollectionViewLayout:(nonnull NSCollectionViewLayout *)layout
{
...
- (CGSize)_sizeForUIKitCellWithKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
  ...
  NSCollectionViewLayout *l = self.collectionViewLayout;
...
- (void)_superPerformBatchUpdates:(void(^)())updates completion:(void(^)(BOOL finished))completion
{
  ...
  [super performBatchUpdates:updates completionHandler:completion];
  _superBatchUpdateCount--;
}
...
- (void)performBatchUpdates:(NS_NOESCAPE void (^)())updates completion:(void (^)(BOOL))completion
{
  ...
  [self performBatchAnimated:true updates:updates completion:completion];
}
...
- (void)registerSupplementaryNodeOfKind:(NSString *)elementKind
{
  ...
  [self registerClass:[_ASCollectionReusableView class] forSupplementaryViewOfKind:elementKind withIdentifier:kReuseIdentifier];
}
...
- (BOOL)beginInteractiveMovementForItemAtIndexPath:(NSIndexPath *)indexPath {
  BOOL success = false /*[super beginInteractiveMovementForItemAtIndexPath:indexPath]*/;
  _reordering = success;
  return success;
}
...
- (CGSize)collectionView:(NSCollectionView *)collectionView layout:(NSCollectionViewLayout *)layout
...
- (CGSize)collectionView:(NSCollectionView *)cv layout:(NSCollectionViewLayout *)l
                       referenceSizeForHeaderInSection:(NSInteger)section
{
  ASDisplayNodeAssertMainThread();
  ASElementMap *map = _dataController.visibleMap;
  ASCollectionElement *e = [map supplementaryElementOfKind:NSCollectionElementKindSectionHeader
                       ...
                       referenceSizeForFooterInSection:(NSInteger)section
{
  ASDisplayNodeAssertMainThread();
  ASElementMap *map = _dataController.visibleMap;
  ASCollectionElement *e = [map supplementaryElementOfKind:NSCollectionElementKindSectionFooter
                                               atIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (NSEdgeInsets)collectionView:(NSCollectionView *)cv layout:(NSCollectionViewLayout *)l
                                      insetForSectionAtIndex:(NSInteger)section
{
  ...
  return ASFlowLayoutDefault(l, sectionInset, NSEdgeInsetsZero);
}

- (CGFloat)collectionView:(NSCollectionView *)cv layout:(NSCollectionViewLayout *)l
...
- (NSCollectionView *)collectionView:(NSCollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
  if ([_registeredSupplementaryKinds containsObject:kind] == NO) {
    [self registerSupplementaryNodeOfKind:kind];
  }
  
  NSCollectionView *view = nil;
...
- (NSCollectionViewItem *)collectionView:(NSCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
  NSCollectionViewItem *cell = nil;
...
//    cell = [self dequeueReusableCellWithReuseIdentifier:kReuseIdentifier forIndexPath:indexPath];
  ...
  if (_ASCollectionViewCell *asCell = ASDynamicCastStrict(cell, _ASCollectionViewCell)) {
    ...
    [_rangeController configureContentView:cell.view forCellNode:node];
  }
  
  return cell;
}

- (void)collectionView:(NSCollectionView *)collectionView willDisplayCell:(NSCollectionViewItem *)rawCell forItemAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  cellNode.scrollView = collectionView.enclosingScrollView;
...
- (void)collectionView:(NSCollectionView *)collectionView didEndDisplayingCell:(NSCollectionViewItem *)rawCell forItemAtIndexPath:(NSIndexPath *)indexPath
{
...
- (void)collectionView:(NSCollectionView *)collectionView willDisplaySupplementaryView:(NSCollectionView *)rawView forElementKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath
{
...
- (void)collectionView:(NSCollectionView *)collectionView didEndDisplayingSupplementaryView:(NSCollectionView *)rawView forElementOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath
{
...
- (void)collectionView:(NSCollectionView *)collectionView didSelectItemAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (void)collectionView:(NSCollectionView *)collectionView didDeselectItemAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (void)collectionView:(NSCollectionView *)collectionView didHighlightItemAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (void)collectionView:(NSCollectionView *)collectionView didUnhighlightItemAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (BOOL)collectionView:(NSCollectionView *)collectionView shouldShowMenuForItemAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (BOOL)collectionView:(NSCollectionView *)collectionView canPerformAction:(nonnull SEL)action forItemAtIndexPath:(nonnull NSIndexPath *)indexPath withSender:(nullable id)sender
{
...
- (void)collectionView:(NSCollectionView *)collectionView performAction:(nonnull SEL)action forItemAtIndexPath:(nonnull NSIndexPath *)indexPath withSender:(nullable id)sender
{
...
- (BOOL)collectionView:(NSCollectionView *)collectionView canMoveItemAtIndexPath:(NSIndexPath *)indexPath
{
...
- (void)collectionView:(NSCollectionView *)collectionView moveItemAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath
{
...
- (void)layoutSubviews
{
  ...
  if (_zeroContentInsets) {
    self.contentInset = NSEdgeInsetsZero;
  }
      ...
      if (shouldReloadData) {
        ...
        [self _superPerformBatchUpdates:^{
          ...
          for (_ASHierarchyItemChange *change in [changeSet itemChangesOfType:_ASHierarchyChangeTypeReload]) {
            NSSet *indexes = [NSSet setWithObjects:change.indexPaths, nil];
            [super reloadItemsAtIndexPaths:indexes];
            numberOfUpdates++;
          }
          ...
          for (_ASHierarchyItemChange *change in [changeSet itemChangesOfType:_ASHierarchyChangeTypeOriginalDelete]) {
            NSSet *indexes = [NSSet setWithObjects:change.indexPaths, nil];
            [super deleteItemsAtIndexPaths:indexes];
            numberOfUpdates++;
          }
          ...
          for (_ASHierarchyItemChange *change in [changeSet itemChangesOfType:_ASHierarchyChangeTypeOriginalInsert]) {
            NSSet *indexes = [NSSet setWithObjects:change.indexPaths, nil];
            [super insertItemsAtIndexPaths:indexes];
            numberOfUpdates++;
          }
...
- (void)nodeSelectedStateDidChange:(ASCellNode *)node
{
  ...
  if (indexPath) {
    NSSet *indexes = [NSSet setWithObject:indexPath];
    if (node.isSelected) {
      [super selectItemsAtIndexPaths:indexes scrollPosition:NSCollectionViewScrollPositionNone];
    } else {
      [super deselectItemsAtIndexPaths:indexes];
    }
  }
}
...
- (void)nodeHighlightedStateDidChange:(ASCellNode *)node
{
  NSIndexPath *indexPath = [self indexPathForNode:node];
  if (indexPath) {
    [self cellForItemAtIndexPath:indexPath].selected = node.isHighlighted;
  }
}
...
//- (BOOL)gestureRecognizer:(NSGestureRecognizer *)gestureRecognizer shouldReceivePress:(NSTouch *)press
//{
