...
#define UITABLEVIEW_RESPONDS_TO_SELECTOR() \
  ({ \
    ...
    dispatch_once(&onceToken, ^{ \
      superResponds = [NSTableView instancesRespondToSelector:_cmd]; \
    }); \
    superResponds; \
  })

@interface NSTableView (ScrollViewDelegate)
...
@interface _ASTableViewCell : NSTableCellView
...
- (void)setElement:(ASCollectionElement *)element
{
  ...
  if (node) {
    self.layer.backgroundColor = node.backgroundColor.CGColor;
...
@interface ASTableView () <ASRangeControllerDataSource, ASRangeControllerDelegate, ASDataControllerSource, _ASTableViewCellDelegate, ASCellNodeInteractionDelegate, ASDelegateProxyInterceptor, ASBatchFetchingScrollView>
{
  ...
  __weak NSView *_sectionIndexView;
...
- (instancetype)initWithFrame:(CGRect)frame style:(NSTableViewStyle)style
{
  return [self _initWithFrame:frame style:style dataControllerClass:nil owningNode:nil];
}

- (instancetype)_initWithFrame:(CGRect)frame style:(NSTableViewStyle)style dataControllerClass:(Class)dataControllerClass owningNode:(ASTableNode *)tableNode
{
  ...
  super.delegate = (id<NSTableViewDelegate>)_proxyDelegate;
  
  _proxyDataSource = [[ASTableViewProxy alloc] initWithTarget:nil interceptor:self];
  super.dataSource = (id<NSTableViewDataSource>)_proxyDataSource;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
  ...
  return [self initWithFrame:CGRectZero style:NSTableViewStylePlain];
}
...
- (nullable NSIndexPath *)validateIndexPath:(nullable NSIndexPath *)indexPath
{
  ...
  if (section >= 2) {
    ASDisplayNodeFailAssert(@"Table view index path has invalid section %lu, section count = %lu", (unsigned long)section, (unsigned long)self.numberOfRows);
    return nil;
  }
  ...
  if (item != NSNotFound) {
    ASDisplayNodeFailAssert(@"Table view index path has invalid item %lu in section %lu, item count = %lu", (unsigned long)item);
    return nil;
  }
...
- (NSInteger)validateRow:(NSInteger)row
{
  // item == NSNotFound means e.g. "scroll to this section" and is acceptable
  if (row != NSNotFound) {
    ASDisplayNodeFailAssert(@"Table view index path has invalid item %lu in section %lu, item count = %lu", (unsigned long)row);
    return row;
  }
...
//  indexPath = [self validateRow:indexPath.item];
...
- (void)endUpdatesWithCompletion:(void (^)(BOOL completed))completion
{
  ...
  [self endUpdatesAnimated:false completion:completion];
}
...
- (void)moveSection:(NSInteger)section toSection:(NSInteger)newSection
{
  ASDisplayNodeAssertMainThread();
  [self beginUpdates];
  [_changeSet moveSection:section toSection:newSection animationOptions:NSTableViewAnimationEffectNone];
  [self endUpdates];
}
...
- (void)reloadRowsAtIndexPaths:(NSArray *)indexPaths withRowAnimation:(NSTableViewAnimationOptions)animation
{
  ASDisplayNodeAssertMainThread();
  if (indexPaths.count == 0) { return; }
  [self beginUpdates];
...
- (void)moveRowAtIndexPath:(NSIndexPath *)indexPath toIndexPath:(NSIndexPath *)newIndexPath
{
  ASDisplayNodeAssertMainThread();
  [self beginUpdates];
  [_changeSet moveItemAtIndexPath:indexPath toIndexPath:newIndexPath animationOptions:NSTableViewAnimationEffectNone];
  [self endUpdates];
}
...
- (void)beginAdjustingContentOffset
{
  ...
  if (firstVisibleIndexPath) {
    ...
    if (node) {
      ...
      _contentOffsetAdjustmentTopVisibleNodeOffset = [self rectOfRow:firstVisibleIndexPath.item].origin.y - self.bounds.origin.y;
    }
  }
}
...
- (void)endAdjustingContentOffsetAnimated:(BOOL)animated
{
  ...
  CGFloat newRowOriginYInSelf = [self rectOfRow:newIndexPathForTopVisibleRow.item].origin.y - self.bounds.origin.y;
...
//  [self.tableHeaderView.asyncdisplaykit_node exitHierarchyState:ASHierarchyStateRangeManaged];
  [super setHeaderView:tableHeaderView];
...
{
  _ASTableViewCell *cell = [self makeViewWithIdentifier:kCellReuseIdentifier owner:self];
  cell.delegate = self;
  ...
  ASCellNode *node = element.node;
  if (node) {
    [_rangeController configureContentView:cell forCellNode:node];
  }
...
- (void)tableView:(NSTableView *)tableView willDisplayCell:(_ASTableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  cellNode.scrollView = tableView.enclosingScrollView;
...
- (void)tableView:(NSTableView *)tableView didEndDisplayingCell:(_ASTableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
...
- (NSIndexPath *)tableView:(NSTableView *)tableView willSelectRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (void)tableView:(NSTableView *)tableView didSelectRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (NSIndexPath *)tableView:(NSTableView *)tableView willDeselectRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (void)tableView:(NSTableView *)tableView didDeselectRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (BOOL)tableView:(NSTableView *)tableView shouldHighlightRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (void)tableView:(NSTableView *)tableView didHighlightRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (void)tableView:(NSTableView *)tableView didUnhighlightRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
  if (_asyncDelegateFlags.tableNodeDidHighlightRow) {
    GET_TABLENODE_OR_RETURN(tableNode, (void)0);
    indexPath = [self convertIndexPathToTableNode:indexPath];
...
- (BOOL)tableView:(NSTableView *)tableView shouldShowMenuForRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
...
- (BOOL)tableView:(NSTableView *)tableView canPerformAction:(nonnull SEL)action forRowAtIndexPath:(nonnull NSIndexPath *)indexPath withSender:(nullable id)sender
{
...
- (void)tableView:(NSTableView *)tableView performAction:(nonnull SEL)action forRowAtIndexPath:(nonnull NSIndexPath *)indexPath withSender:(nullable id)sender
{
...
- (void)scrollViewWillEndDragging:(NSScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
  ...
  CGPoint contentOffset = scrollView.contentView.documentVisibleRect.origin;
...
//  }
  ...
  return scrollDirection; /*ASScrollDirectionApplyTransform(scrollDirection, self.transform);*/
}
...
- (ASScrollDirection)scrollableDirections
{
  ...
  CGFloat totalContentWidth = self.intrinsicContentSize.width + self.contentInset.left + self.contentInset.right;
  CGFloat totalContentHeight = self.intrinsicContentSize.height + self.contentInset.top + self.contentInset.bottom;
  
  if (totalContentWidth > self.bounds.size.width) { // Can scroll horizontally.
    scrollableDirection |= ASScrollDirectionHorizontalDirections;
  }
  if (totalContentHeight > self.bounds.size.height) { // Can scroll vertically.
...
{
  if (ASDisplayShouldFetchBatchForScrollView(self.enclosingScrollView, self.scrollDirection, ASScrollDirectionVerticalDirections, contentOffset, velocity, NO)) {
...
- (void)rangeController:(ASRangeController *)rangeController updateWithChangeSet:(_ASHierarchyChangeSet *)changeSet updates:(dispatch_block_t)updates
{
  ...
  for (_ASHierarchyItemChange *change in [changeSet itemChangesOfType:_ASHierarchyChangeTypeReload]) {
    ...
    NSInteger targetSection = 0; // Укажите нужный раздел
    NSMutableIndexSet *indexSet = [NSMutableIndexSet indexSet];
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.section == targetSection) {
          [indexSet addIndex:indexPath.item];
        }
    }
    NSTableViewAnimationOptions animationOptions = (NSTableViewAnimationOptions)change.animationOptions;
    
    LOG(@"UITableView reloadRows:%ld rows", indexPaths.count);
    BOOL preventAnimation = animationOptions == NSTableViewAnimationEffectNone;
    ASPerformBlockWithoutAnimation(preventAnimation, ^{
      if (self.test_enableSuperUpdateCallLogging) {
        NSLog(@"-[super reloadRowsAtIndexPaths]: %@", indexPaths);
      }
      [super reloadDataForRowIndexes:indexSet columnIndexes:indexSet];
    });
  ...
  for (_ASHierarchySectionChange *change in [changeSet sectionChangesOfType:_ASHierarchyChangeTypeReload]) {
    ...
    ASPerformBlockWithoutAnimation(preventAnimation, ^{
      ...
      [super reloadDataForRowIndexes:sectionIndexes columnIndexes:sectionIndexes];
    });
  ...
  for (_ASHierarchyItemChange *change in [changeSet itemChangesOfType:_ASHierarchyChangeTypeOriginalDelete]) {
    ...
    ASPerformBlockWithoutAnimation(preventAnimation, ^{
      ...
      [super removeRowsAtIndexes:indexSet withAnimation:animationOptions];
    });
  ...
  for (_ASHierarchySectionChange *change in [changeSet sectionChangesOfType:_ASHierarchyChangeTypeOriginalDelete]) {
    ...
    ASPerformBlockWithoutAnimation(preventAnimation, ^{
      ...
      [super removeRowsAtIndexes:sectionIndexes withAnimation:animationOptions];
    });
  ...
  for (_ASHierarchySectionChange *change in [changeSet sectionChangesOfType:_ASHierarchyChangeTypeOriginalInsert]) {
    ...
    ASPerformBlockWithoutAnimation(preventAnimation, ^{
      ...
      [super insertRowsAtIndexes:sectionIndexes withAnimation:animationOptions];
    });
  ...
  for (_ASHierarchyItemChange *change in [changeSet itemChangesOfType:_ASHierarchyChangeTypeOriginalInsert]) {
    ...
    ASPerformBlockWithoutAnimation(preventAnimation, ^{
      ...
      [super insertRowsAtIndexes:indexSet withAnimation:animationOptions];
    });
...
- (BOOL)dataController:(ASDataController *)dataController presentedSizeForElement:(ASCollectionElement *)element matchesSize:(CGSize)size
{
  ...
  CGRect rect = [self rectOfRow:indexPath.item];
...
- (void)didLayoutSubviewsOfTableViewCell:(_ASTableViewCell *)tableViewCell
{
  ...
  CGFloat contentViewWidth = tableViewCell.bounds.size.width;
...
//      [self selectRowAtIndexPath:indexPath animated:NO scrollPosition:UITableViewScrollPositionNone];
      ...
      [self deselectRow:indexPath.item];
    }
  }
}
...
- (CGFloat)sectionIndexWidth
{
  ...
  NSView *indexView = _sectionIndexView;
...
{
  [self waitUntilAllUpdatesAreCommitted];
  return [super accessibilityWindows];
}
