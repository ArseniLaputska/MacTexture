...
- (void)_locked_setDefaultImage:(NSImage *)image;
@end
...
- (void)_locked_setAnimatedImage:(id <ASAnimatedImageProtocol>)animatedImage
{
  ...
  if (animatedImage != nil) {
    ...
    if ([animatedImage respondsToSelector:@selector(setCoverImageReadyCallback:)]) {
      animatedImage.coverImageReadyCallback = ^(NSImage *coverImage) {
...
- (void)setCoverImageCompleted:(NSImage *)coverImage
{
...
- (void)_locked_setCoverImageCompleted:(NSImage *)coverImage
{
  DISABLED_ASAssertLocked(__instanceLock__);
...
- (void)_locked_setCoverImage:(NSImage *)coverImage
{
  DISABLED_ASAssertLocked(__instanceLock__);
...
- (void)_locked_startAnimating
{
  ...
  if (_displayLink == nil) {
    _playHead = 0;
    _displayLink = [[NSScreen mainScreen] displayLinkWithTarget:[ASWeakProxy weakProxyWithTarget:self] selector:@selector(displayLinkFired:)];
