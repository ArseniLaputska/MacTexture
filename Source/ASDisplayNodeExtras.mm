...
ASInterfaceState ASInterfaceStateForDisplayNode(ASDisplayNode *displayNode, NSWindow *window)
{
...
ASDisplayNode *ASViewToDisplayNode(NSView *view)
{
...
NSWindow * _Nullable ASFindWindowOfLayer(CALayer *layer)
{
  NSView *view = ASFindClosestViewOfLayer(layer);
  if (NSWindow *window = ASDynamicCast(view, NSWindow)) {
    return window;
  } else {
    return view.window;
  }
...
NSView * _Nullable ASFindClosestViewOfLayer(CALayer *layer)
{
  while (layer != nil) {
    if (NSView *view = ASDynamicCast(layer.delegate, NSView)) {
      return view;
    }
...
NSColor *ASDisplayNodeDefaultPlaceholderColor()
{
  static NSColor *defaultPlaceholderColor;

  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    defaultPlaceholderColor = [NSColor colorWithWhite:0.95 alpha:1.0];
  });
  return defaultPlaceholderColor;
}

NSColor *ASDisplayNodeDefaultTintColor()
{
  static NSColor *defaultTintColor;

  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    defaultTintColor = [NSColor colorWithRed:0.0 green:0.478 blue:1.0 alpha:1.0];
  });
