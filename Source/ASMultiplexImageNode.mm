 ...
 */
typedef void(^ASMultiplexImageLoadCompletionBlock)(NSImage *image, id imageIdentifier, NSError *error);
...
/**
  ...
  @result The best NSImage available immediately; nil if no image is immediately available.
 */
- (NSImage *)_bestImmediatelyAvailableImageFromDataSource:(id *)imageIdentifierOut;
 ...
 */
- (void)_fetchImageWithIdentifierFromCache:(id)imageIdentifier URL:(NSURL *)imageURL completion:(void (^)(NSImage *image))completionBlock;
 ...
 */
- (void)_loadALAssetWithIdentifier:(id)imageIdentifier URL:(NSURL *)assetURL completion:(void (^)(NSImage *image, NSError *error))completionBlock;
#endif
...
/**
  ...
  @param completionBlock The block to be performed when the image has been loaded, if possible. May not be nil.
 */
- (void)_loadPHAssetWithRequest:(ASPhotosFrameworkImageRequest *)request identifier:(id)imageIdentifier completion:(void (^)(NSImage *image, NSError *error))completionBlock API_AVAILABLE(ios(8.0), tvos(10.0));
#endif
 ...
 */
- (void)_downloadImageWithIdentifier:(id)imageIdentifier URL:(NSURL *)imageURL completion:(void (^)(NSImage *image, NSError *error))completionBlock;
...
- (void)displayDidFinish
{
  ...
  NSImage *displayedImage = self.image;
  if (displayedImage) {
...
- (void)_setDisplayedImageIdentifier:(id)displayedImageIdentifier withImage:(NSImage *)image
{
...
- (void)_loadImageIdentifiers
{
  ...
  NSImage *bestImmediatelyAvailableImage = [self _bestImmediatelyAvailableImageFromDataSource:&bestImmediatelyAvailableImageIdentifier];
...
- (NSImage *)_bestImmediatelyAvailableImageFromDataSource:(id *)imageIdentifierOut
{
  ...
  NSImage *existingImage = self.image;
  for (id imageIdentifier in _imageIdentifiers) {
    ...
    // the data source may generate a new instance of NSImage that returns NO for isEqual:
    // and we'll end up in an infinite loading loop.
    NSImage *image = ASObjectIsEqual(imageIdentifier, _loadedImageIdentifier) ? existingImage : [_dataSource multiplexImageNode:self imageForImageIdentifier:imageIdentifier];
    if (image) {
...
- (void)_updateProgressImageBlockOnDownloaderIfNeeded
{
  ...
  if (shouldRenderProgressImages && ASInterfaceStateIncludesVisible(interfaceState)) {
    __weak __typeof__(self) weakSelf = self;
    progress = ^(NSImage * _Nonnull progressImage, CGFloat progress, id _Nullable downloadIdentifier) {
      __typeof__(self) strongSelf = weakSelf;
      if (strongSelf == nil) {
        return;
      }
...
- (void)_loadNextImage
{
  ...
  __weak __typeof__(self) weakSelf = self;
  ASMultiplexImageLoadCompletionBlock finishedLoadingBlock = ^(NSImage *image, id imageIdentifier, NSError *error) {
    __typeof__(self) strongSelf = weakSelf;
    if (!strongSelf)
      return;
  ...
  if (_dataSourceFlags.image) {
    NSImage *image = [_dataSource multiplexImageNode:self imageForImageIdentifier:nextImageIdentifier];
    if (image) {
  ...
  if ([[nextImageURL scheme] isEqualToString:kAssetsLibraryURLScheme]) {
    // Load the asset.
    [self _loadALAssetWithIdentifier:nextImageIdentifier URL:nextImageURL completion:^(NSImage *downloadedImage, NSError *error) {
  ...
  if (AS_AVAILABLE_IOS_TVOS(9, 10)) {
    ...
    if (ASPhotosFrameworkImageRequest *request = [ASPhotosFrameworkImageRequest requestWithURL:nextImageURL]) {
      [self _loadPHAssetWithRequest:request identifier:nextImageIdentifier completion:^(NSImage *image, NSError *error) {
  ...
  [self _fetchImageWithIdentifierFromCache:nextImageIdentifier URL:nextImageURL completion:^(NSImage *imageFromCache) {
    __typeof__(self) strongSelf = weakSelf;
    if (!strongSelf)
      return;
    ...
    [strongSelf _downloadImageWithIdentifier:nextImageIdentifier URL:nextImageURL completion:^(NSImage *downloadedImage, NSError *error) {
      __typeof__(self) strongSelf = weakSelf;
      if (downloadedImage) {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  [assetLibrary assetForURL:assetURL resultBlock:^(ALAsset *asset) {
    ...
    NSImage *downloadedImage = (coreGraphicsImage ? [NSImage imageWithCGImage:coreGraphicsImage] : nil);
...
#if AS_USE_PHOTOS
- (void)_loadPHAssetWithRequest:(ASPhotosFrameworkImageRequest *)request identifier:(id)imageIdentifier completion:(void (^)(NSImage *image, NSError *error))completionBlock
{
  ASDisplayNodeAssertNotNil(imageIdentifier, @"imageIdentifier is required");
  ...
  NSOperation *newImageRequestOp = [NSBlockOperation blockOperationWithBlock:^{
    ...
    [imageManager requestImageForAsset:imageAsset targetSize:request.targetSize contentMode:request.contentMode options:options resultHandler:^(NSImage *image, NSDictionary *info) {
...
#pragma mark -
- (void)_finishedLoadingImage:(NSImage *)image forIdentifier:(id)imageIdentifier error:(NSError *)error
{
  ...
  if (image || imageIdentifierCount == 0) {
    ...
    NSImage *previousImage = self.image;
