...
@interface ASDisplayNode () <NSGestureRecognizerDelegate, _ASDisplayLayerDelegate, ASCATransactionQueueObserving>
/**
...
//  _defaultLayoutTransitionOptions = NSViewAnimationOptionCurveEaseInOut | NSViewAnimationOptionTransitionNone;
  
  _flags.canClearContentsOfLayer = YES;
  _flags.canCallSetNeedsDisplayOfLayer = YES;

  _fallbackSafeAreaInsets = NSEdgeInsetsZero;
...
- (instancetype)initWithViewClass:(Class)viewClass
{
  ...
  ASDisplayNodeAssert([viewClass isSubclassOfClass:[NSView class]], @"should initialize with a subclass of UIView");
  ...
  if (primitiveTraitCollection.userInterfaceStyle != previousTraitCollection.userInterfaceStyle) {
    if (loaded) {
      ...
      ASPerformBlockOnMainThread(^{
        ...
        NSColor *backgroundColor = self->_backgroundColor;
        ...
        if (!CGColorEqualToColor(self->_layer.backgroundColor, cgBackgroundColor)) {
          ...
          // instead of NSColor. Non layer backed node also receive color to the layer (see [_ASPendingState -applyToView:withSpecialPropertiesHandling:]).
...
- (NSView *)_locked_viewToLoad
{
  ...
  if (checkFlag(Synchronous)) {
    ...
    // NSImageView layers. More details on the flags
    if ([_viewClass isSubclassOfClass:[NSImageView class]]) {
      _flags.canClearContentsOfLayer = NO;
      _flags.canCallSetNeedsDisplayOfLayer = NO;
    }
      
    // UIActivityIndicator
//    if ([_viewClass isSubclassOfClass:[NSActivityIndicatorView class]]
//        || [_viewClass isSubclassOfClass:[NSVisualEffectView class]]) {
//      self.opaque = NO;
//    }
...
- (NSEdgeInsets)fallbackSafeAreaInsets
{
  MutexLocker l(__instanceLock__);
  return _fallbackSafeAreaInsets;
}

- (void)setFallbackSafeAreaInsets:(NSEdgeInsets)insets
{
...

  {
    ...
    if (NSEdgeInsetsEqualToEdgeInsets(insets, _fallbackSafeAreaInsets)) {
      return;
    }
...
static inline BOOL NSEdgeInsetsEqualToEdgeInsets(NSEdgeInsets insets1, NSEdgeInsets insets2) {
    return NSEqualSizes(NSMakeSize(insets1.left, insets1.top),
                        NSMakeSize(insets2.left, insets2.top)) &&
           NSEqualSizes(NSMakeSize(insets1.right, insets1.bottom),
                        NSMakeSize(insets2.right, insets2.bottom));
}
...
- (void)_fallbackUpdateSafeAreaOnChildren
{
  ...
  for (ASDisplayNode *child in self.subnodes) {
    ...
    NSEdgeInsets childInsets = NSEdgeInsetsMake(MAX(insets.top    - (CGRectGetMinY(childFrame) - CGRectGetMinY(bounds)), 0),
...
- (void)_updateClipCornerLayerContentsWithRadius:(CGFloat)radius backgroundColor:(NSColor *)backgroundColor
{
  ASPerformBlockOnMainThread(^{
    for (int idx = 0; idx < NUM_CLIP_CORNER_LAYERS; idx++) {
      ...
      NSImage *newContents = [[NSImage alloc] initWithSize:size];
      [newContents lockFocus];

      // Получаем текущий графический контекст
      CGContextRef ctx = [NSGraphicsContext currentContext].CGContext;

      // Перемещаем контекст, если это необходимо
      if (isRight == YES) {
      ...
      // Создаем путь с округленными углами
      NSBezierPath *roundedRect = [NSBezierPath bezierPathWithRoundedRect:NSMakeRect(0, 0, radius * 2, radius * 2)
                                                                xRadius:radius
                                                                yRadius:radius];
      
      [roundedRect setWindingRule:NSWindingRuleEvenOdd];

      // Добавляем прямоугольник для создания "дыры" вокруг круглой формы
      [roundedRect appendBezierPathWithRect:NSMakeRect(-1, -1, radius * 2 + 1, radius * 2 + 1)];
      ...
      CGImageRef cgImage = [newContents CGImageForProposedRect:NULL context:nil hints:nil];
      clipCornerLayer.contents = (__bridge id)cgImage;
...
- (CGPoint)convertPoint:(CGPoint)point fromNode:(ASDisplayNode *)node
{
  ...
  if (node == nil && self.nodeLoaded) {
    CALayer *layer = self.layer;
    if (NSWindow *window = ASFindWindowOfLayer(layer)) {
      return [layer convertPoint:point fromLayer:window.contentView.layer];
    } else {
      return point;
    }
  }
...
- (CGPoint)convertPoint:(CGPoint)point toNode:(ASDisplayNode *)node
{
  ...
  if (node == nil && self.nodeLoaded) {
    CALayer *layer = self.layer;
    if (NSWindow *window = ASFindWindowOfLayer(layer)) {
      return [layer convertPoint:point toLayer:window.contentView.layer];
    } else {
      return point;
    }
  }
...
- (CGRect)convertRect:(CGRect)rect fromNode:(ASDisplayNode *)node
{
  ...
  if (node == nil && self.nodeLoaded) {
    CALayer *layer = self.layer;
    if (NSWindow *window = ASFindWindowOfLayer(layer)) {
      return [layer convertRect:rect fromLayer:window.contentView.layer];
    } else {
      return rect;
    }
  }
...
- (CGRect)convertRect:(CGRect)rect toNode:(ASDisplayNode *)node
{
  ...
  if (node == nil && self.nodeLoaded) {
    CALayer *layer = self.layer;
    if (NSWindow *window = ASFindWindowOfLayer(layer)) {
      return [layer convertRect:rect toLayer:window.contentView.layer];
    } else {
      return rect;
    }
  }
...
- (void)_removeFromSupernode:(ASDisplayNode *)supernode view:(NSView *)view layer:(CALayer *)layer
{
...
- (void)_locked_setupPlaceholderLayerIfNeeded
{
  ...
  if (_placeholderLayer.contents == nil) {
    ...
    if (_placeholderImage) {
      BOOL stretchable = !NSEdgeInsetsEqualToEdgeInsets(_placeholderImage.capInsets, NSEdgeInsetsZero);
      if (stretchable) {
        ASDisplayNodeSetResizableContents(_placeholderLayer, _placeholderImage);
      } else {
        _placeholderLayer.contentsScale = self.contentsScale;
        CGImageRef cgImage = [_placeholderImage CGImageForProposedRect:NULL context:nil hints:nil];
        _placeholderLayer.contents = (__bridge id)cgImage;
      }
    }
  }
}
...
//  return [superview gesture gestureRecognizerShouldBegin:gestureRecognizer];
//}

- (NSView *)hitTest:(CGPoint)point
{
  ASDisplayNodeAssertMainThread();
  return [_view hitTest:point];
}

- (void)setHitTestSlop:(NSEdgeInsets)hitTestSlop
{
  MutexLocker l(__instanceLock__);
  _hitTestSlop = hitTestSlop;
}

- (NSEdgeInsets)hitTestSlop
{
  MutexLocker l(__instanceLock__);
  return _hitTestSlop;
}
...
//{
//  ASDisplayNodeAssertMainThread();
//  NSEdgeInsets slop = self.hitTestSlop;
//  if (_view && NSEdgeInsetsEqualToEdgeInsets(slop, NSEdgeInsetsZero)) {
//    // Safer to use UIView's -pointInside:withEvent: if we can.
//    return [_view  pointInside:point withEvent:event];
//  } else {
//    return CGRectContainsPoint(NSEdgeInsetsInsetRect(self.bounds, slop), point);
//  }
//}
...
- (NSMutableArray<NSDictionary *> *)propertiesForDebugDescription
{
  ...
  if (CGRectIsNull(windowFrame) == NO) {
    [result addObject:@{ @"frameInWindow" : [NSValue valueWithRect:windowFrame] }];
  }
  ...
  if (_view != nil) {
    for (NSResponder *responder in [_view asdk_responderChainEnumerator]) {
  ...
  if (_view != nil) {
    [result addObject:@{ @"alphaValue" : @(_view.alphaValue) }];
    [result addObject:@{ @"frame" : [NSValue valueWithRect:_view.frame] }];
  } else if (_layer != nil) {
    [result addObject:@{ @"alpha" : @(_layer.opacity) }];
    [result addObject:@{ @"frame" : [NSValue valueWithRect:_layer.frame] }];
  } else if (_pendingViewState != nil) {
    [result addObject:@{ @"alpha" : @(_pendingViewState.alpha) }];
    [result addObject:@{ @"frame" : [NSValue valueWithRect:_pendingViewState.frame] }];
  }
...
- (NSString *)detailedLayoutDescription
{
  ...
  [props addObject:@{ @"bounds": [NSValue valueWithRect:self.bounds] }];
