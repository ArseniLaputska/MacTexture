...
static AppState __ApplicationState = AppStateActive;
...
- (ASInterfaceState)interfaceState
{
  ...
  if (__ApplicationState == AppStateBackground) {
...
- (void)configureContentView:(NSView *)contentView forCellNode:(ASCellNode *)node
{
...
+ (ASWeakSet *)allRangeControllersWeakSet
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    [self setupApplicationStateTracking];
...
+ (void)registerSharedApplicationNotifications
{
  ...
  [center addObserver:self selector:@selector(didReceiveMemoryWarning:) name:NSApplicationWillTerminateNotification object:nil];
#endif
...
+ (void)setupApplicationStateTracking {
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)applicationDidBecomeActive {
    __ApplicationState = AppStateActive;
}

- (void)applicationDidResignActive {
    __ApplicationState = AppStateInactive;
}

- (void)applicationWillTerminate {
    __ApplicationState = AppStateBackground;
}
