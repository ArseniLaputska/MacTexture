...
void SortAccessibilityElements(NSMutableArray *elements)
{
  ...
  dispatch_once(&onceToken, ^{
    defaultAccessibilityComparator = ^NSComparisonResult(NSObject *a, NSObject *b) {
      if (([a isKindOfClass:[NSView class]]) & ([b isKindOfClass:[NSView class]])) {
        NSView *aView = (NSView *)a;
        NSView *bView = (NSView *)b;
        
        CGPoint originA = aView.accessibilityFrame.origin;
        CGPoint originB = bView.accessibilityFrame.origin;
        if (originA.y == originB.y) {
          if (originA.x == originB.x) {
            ...
            CGSize sizeA = aView.accessibilityFrame.size;
            CGSize sizeB = bView.accessibilityFrame.size;
...
static CGRect ASAccessibilityFrameForNode(ASDisplayNode *node) {
  ...
  return [layer convertRect:node.bounds toLayer:ASFindWindowOfLayer(layer).contentView.layer];
}

@interface ASAccessibilityElement : NSObject <NSAccessibility>
...
+ (ASAccessibilityElement *)accessibilityElementWithContainer:(NSView *)container node:(ASDisplayNode *)node;
...
+ (ASAccessibilityElement *)accessibilityElementWithContainer:(NSView *)container node:(ASDisplayNode *)node
{
    ASAccessibilityElement *accessibilityElement = [[ASAccessibilityElement alloc] init];
    accessibilityElement.container = container;
...
@interface ASAccessibilityCustomAction : NSAccessibilityCustomAction
...
static void CollectUIAccessibilityElementsForNode(ASDisplayNode *node, ASDisplayNode *containerNode, id container, NSMutableArray *elements)
{
  ...
  ASDisplayNodePerformBlockOnEveryNodeBFS(node, ^(ASDisplayNode * _Nonnull currentNode) {
    ...
    if (currentNode != containerNode && currentNode.isAccessibilityElement) {
      ASAccessibilityElement *accessibilityElement = [ASAccessibilityElement accessibilityElementWithContainer:container node:currentNode];
      [elements addObject:accessibilityElement];
    }
  });
}

static void CollectAccessibilityElementsForContainer(ASDisplayNode *container, NSView *view,
                                                     NSMutableArray *elements) {
  ASDisplayNodeCAssertNotNil(view, @"Passed in view should not be nil");
  if (view == nil) {
    return;
  }
  ASAccessibilityElement *accessiblityElement =
  ...
  while (!queue.empty()) {
    ...
    if (node != container && node.isAccessibilityContainer) {
      NSView *containerView = node.isLayerBacked ? view : node.view;
...
//      }
     if (node == container || shouldAggregateSubnodeLabels) {
...
static BOOL recusivelyCheckSuperviewsForScrollView(NSView *view) {
    if (!view) {
        return NO;
  ...
  for (ASDisplayNode *subnode in subnodes) {
    ...
    if (subnode.isAccessibilityElement) {
      ...
      if (subnode.isLayerBacked) {
        ...
        ASAccessibilityElement *accessiblityElement = [ASAccessibilityElement accessibilityElementWithContainer:view node:subnode];
        [elements addObject:accessiblityElement];
      } else {
...
- (nullable NSArray *)accessibilityElements
{
  ...
  NSArray *elements = [super accessibilityAttributeNames];
  if (elements.count) {
    return elements;
  }
...
//@implementation _ASDisplayView (NSAccessibilityAction)
//
...
//}
//
//- (BOOL)accessibilityScroll:(NSAccessibilityScrollDirection)direction {
