...
@interface ASScrollView : NSScrollView
@end
...
{
  if (self = [super init]) {
    [self setViewBlock:^NSView *{ return [[ASScrollView alloc] init]; }];
  }
  return self;
}
...
- (void)layout
{
  ...
  if (_automaticallyManagesContentSize) {
    ...
    if (ASIsCGSizeValidForLayout(contentSize) == NO) {
      NSLog(@"%@ calculated a size in its layout spec that can't be applied to .contentSize: %@. Applying parentSize (scrollNode's bounds) instead: %@.", self, NSStringFromSize(contentSize), NSStringFromSize(self.calculatedSize));
