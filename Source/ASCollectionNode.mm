...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(NSCollectionViewLayout *)layout layoutFacilitator:(id<ASCollectionViewLayoutFacilitatorProtocol>)layoutFacilitator
{
  if (self = [super init]) {
...
- (void)didLoad
{
  ...
  if (_pendingState) {
    ...
    view.allowsEmptySelection           = pendingState.allowsSelection;
...
//    }
    ...
    NSEdgeInsets contentInset = pendingState.contentInset;
    if (!NSEdgeInsetsEqualToEdgeInsets(contentInset, NSEdgeInsetsZero)) {
...
- (void)setAllowsSelection:(BOOL)allowsSelection
{
  if ([self pendingState]) {
    ...
    ASDisplayNodeAssert([self isNodeLoaded], @"ASCollectionNode should be loaded if pendingState doesn't exist");
    self.view.allowsEmptySelection = allowsSelection;
  }
}
...
- (BOOL)allowsSelection
{
  if ([self pendingState]) {
    ...
    return self.view.allowsEmptySelection;
  }
}
...
- (void)deselectItemAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated
{
  ...
  indexPath = [collectionView convertIndexPathFromCollectionNode:indexPath waitingIfNeeded:YES];
  NSSet *indexSet = [NSSet setWithObject:indexPath];

  if (indexPath != nil) {
    [collectionView deselectItemsAtIndexPaths:indexSet];
  } else {
...
{
  [self performBatchAnimated:false updates:updates completion:completion];
}
...
- (void)_configureCollectionViewLayout:(NSCollectionViewLayout *)layout
{
  if ([layout isKindOfClass:[ASCollectionLayout class]]) {
