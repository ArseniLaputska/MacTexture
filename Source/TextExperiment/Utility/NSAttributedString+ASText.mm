...
- (NSFont *)as_fontAtIndex:(NSUInteger)index {
...
- (NSColor *)as_colorAtIndex:(NSUInteger)index {
  NSColor *color = [self as_attribute:NSForegroundColorAttributeName atIndex:index];
  if (!color) {
...
- (NSColor *)as_backgroundColor {
  return [self as_backgroundColorAtIndex:0];
}

- (NSColor *)as_backgroundColorAtIndex:(NSUInteger)index {
...
- (NSColor *)as_strokeColor {
  return [self as_strokeColorAtIndex:0];
}

- (NSColor *)as_strokeColorAtIndex:(NSUInteger)index {
  NSColor *color = [self as_attribute:NSStrokeColorAttributeName atIndex:index];
  if (!color) {
...
- (NSColor *)as_strikethroughColor {
  return [self as_strikethroughColorAtIndex:0];
}

- (NSColor *)as_strikethroughColorAtIndex:(NSUInteger)index {
...
- (NSColor *)as_underlineColor {
  return [self as_underlineColorAtIndex:0];
}

- (NSColor *)as_underlineColorAtIndex:(NSUInteger)index {
  NSColor *color = [self as_attribute:NSUnderlineColorAttributeName atIndex:index];
  if (!color) {
...
- (CGAffineTransform)as_textGlyphTransformAtIndex:(NSUInteger)index {
  ...
  return [value as_CGAffineTransformValue];
}
...
+ (NSMutableAttributedString *)as_attachmentStringWithEmojiImage:(NSImage *)image
                                                        fontSize:(CGFloat)fontSize {
  if (!image || fontSize <= 0) return nil;
  
  BOOL hasAnim = NO;
  if (NSProtocolFromString(@"ASAnimatedImage") &&
  ...
  if (hasAnim) {
    ...
    if (!imageClass) imageClass = [NSImageView class];
    NSImageView *view = (id)[imageClass new];
    view.frame = bounding;
    view.image = image;
...
- (void)setAs_color:(NSColor *)color {
  [self as_setColor:color range:NSMakeRange(0, self.length)];
}

- (void)setAs_backgroundColor:(NSColor *)backgroundColor {
...
- (void)setAs_strokeColor:(NSColor *)strokeColor {
...
- (void)setAs_strikethroughColor:(NSColor *)strikethroughColor {
...
- (void)setAs_underlineColor:(NSColor *)underlineColor {
...
- (void)as_setFont:(NSFont *)font range:(NSRange)range {
...
- (void)as_setColor:(NSColor *)color range:(NSRange)range {
...
- (void)as_setBackgroundColor:(NSColor *)backgroundColor range:(NSRange)range {
...
- (void)as_setStrokeColor:(NSColor *)strokeColor range:(NSRange)range {
...
- (void)as_setStrikethroughColor:(NSColor *)strikethroughColor range:(NSRange)range {
...
- (void)as_setUnderlineColor:(NSColor *)underlineColor range:(NSRange)range {
...
- (void)as_setTextGlyphTransform:(CGAffineTransform)textGlyphTransform range:(NSRange)range {
  NSValue *value = CGAffineTransformIsIdentity(textGlyphTransform) ? nil : [NSValue as_valueWithCGAffineTransform:textGlyphTransform];
