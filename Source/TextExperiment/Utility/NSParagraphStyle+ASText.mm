...
#pragma clang diagnostic pop
  ...
  if (CTParagraphStyleGetValueForSpecifier(CTStyle, kCTParagraphStyleSpecifierAlignment, sizeof(CTTextAlignment), &alignment)) {
    style.alignment = CTTextAlignmentToNSTextAlignment(alignment);
  }
  ...
  if (CTParagraphStyleGetValueForSpecifier(CTStyle, kCTParagraphStyleSpecifierTabStops, sizeof(CFArrayRef), &tabStops)) {
    ...
    [((__bridge NSArray *)(tabStops))enumerateObjectsUsingBlock : ^(id obj, NSUInteger idx, BOOL *stop) {
      ...
      NSTextTab *tab = [[NSTextTab alloc] initWithTextAlignment:CTTextAlignmentToNSTextAlignment(CTTextTabGetAlignment(ctTab)) location:CTTextTabGetLocation(ctTab) options:(__bridge id)CTTextTabGetOptions(ctTab)];
      [tabs addObject:tab];
    }];
    if (tabs.count) {
      style.tabStops = tabs;
...
CTTextAlignment NSTextAlignmentToCTTextAlignment(NSTextAlignment nsTextAlignment) {
    switch (nsTextAlignment) {
        ...
        default:
            return kCTTextAlignmentNatural; // Безопасный дефолт
    }
}

NSTextAlignment CTTextAlignmentToNSTextAlignment(CTTextAlignment ctTextAlignment) {
    switch (ctTextAlignment) {
        ...
        default:
            return NSTextAlignmentNatural; // Безопасный дефолт
    }
}
