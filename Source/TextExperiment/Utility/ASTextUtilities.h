...
/// Negates/inverts a NSEdgeInsets.
static inline NSEdgeInsets ASTextNSEdgeInsetsInvert(NSEdgeInsets insets) {
...
static inline NSFont * _Nullable ASTextFontWithBold(NSFont *font) {
    if (!font) return nil;

    // Создаём дескриптор с нужными символическими признаками (только Bold).
    NSFontDescriptor *desc = [font.fontDescriptor fontDescriptorWithSymbolicTraits:NSFontBoldTrait];
    if (!desc) return nil;
    
    // Создаём новый шрифт
    return [NSFont fontWithDescriptor:desc size:font.pointSize];
}

static inline NSFont * _Nullable ASTextFontWithItalic(NSFont *font) {
    if (!font) return nil;

    // Создаём дескриптор с нужными символическими признаками (только Italic).
    NSFontDescriptor *desc = [font.fontDescriptor fontDescriptorWithSymbolicTraits:NSFontItalicTrait];
    if (!desc) return nil;

    return [NSFont fontWithDescriptor:desc size:font.pointSize];
}

static inline NSFont * _Nullable ASTextFontWithBoldItalic(NSFont *font) {
    if (!font) return nil;

    // Объединяем две константы: Bold + Italic
    NSFontSymbolicTraits traits = NSFontBoldTrait | NSFontItalicTrait;
