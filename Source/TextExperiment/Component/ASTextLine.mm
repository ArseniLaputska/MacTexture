...
- (void)reloadBounds {
  ...
  for (NSUInteger r = 0; r < runCount; r++) {
    ...
    if (attachment) {
      ...
      [attachmentRects addObject:[NSValue valueWithRect:runTypoBounds]];
    }
  }
...
- (NSString *)description {
  ...
  [desc appendFormat:@" position:%@",NSStringFromPoint(self.position)];
  [desc appendFormat:@" bounds:%@",NSStringFromRect(self.bounds)];
  return desc;
}
