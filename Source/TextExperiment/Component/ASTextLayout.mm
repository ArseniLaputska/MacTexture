...
static inline NSEdgeInsets UIEdgeInsetRotateVertical(NSEdgeInsets insets) {
  NSEdgeInsets one;
  one.top = insets.left;
...
/**
 Sometimes CoreText may convert CGColor to NSColor for `kCTForegroundColorAttributeName`
...
static CGColorRef ASTextGetCGColor(CGColorRef color) {
  ...
  if ([((__bridge NSObject *)color) respondsToSelector:@selector(CGColor)]) {
    return ((__bridge NSColor *)color).CGColor;
  }
  return color;
}
...
- (NSString *)description
{
  return [NSString
          stringWithFormat:@"immutable: %@, insets: %@, size: %@", self->_readonly ? @"YES" : @"NO",
            NSStringFromNSEdgeInsets(self->_insets), NSStringFromSize(self->_size)];
}
...
+ (instancetype)containerWithSize:(CGSize)size NS_RETURNS_RETAINED {
  return [self containerWithSize:size insets:NSEdgeInsetsZero];
}

+ (instancetype)containerWithSize:(CGSize)size insets:(NSEdgeInsets)insets NS_RETURNS_RETAINED {
...
+ (instancetype)containerWithPath:(NSBezierPath *)path NS_RETURNS_RETAINED {
  ASTextContainer *one = [self new];
  one.path = path;
  return one;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  [aCoder encodeObject:[NSValue valueWithSize:_size] forKey:@"size"];
  [aCoder encodeObject:[NSValue valueWithEdgeInsets:_insets] forKey:@"insets"];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  self = [self init];
  _size = ((NSValue *)[aDecoder decodeObjectForKey:@"size"]).sizeValue;
...
- (NSEdgeInsets)insets {
  Getter(NSEdgeInsets insets = _insets) return insets;
}
...
- (NSBezierPath *)path {
  Getter(NSBezierPath *path = _path) return path;
}
...
- (NSString *)description
{
  return [NSString stringWithFormat:@"lines: %ld, visibleRange:%@, textBoundingRect:%@",
                                    ...
                                    NSStringFromRect(self.textBoundingRect)];
}
...
#define FAIL_AND_RETURN {\
  ...
  if (container.path == nil && container.exclusionPaths.count == 0) {
    ...
    if (needFixLayoutSizeBug) {
      ...
      constraintRectBeforeExtended = NSEdgeInsetsInsetRect(rect, container.insets);
    ...
    rect = NSEdgeInsetsInsetRect(rect, container.insets);
    ...
    if (path) {
      [layout.container.exclusionPaths enumerateObjectsUsingBlock: ^(NSBezierPath *onePath, NSUInteger idx, BOOL *stop) {
...

  {
    ...
    { // calculate bounding size
      ...
      if (container.path) {
        ...
        rect = NSEdgeInsetsInsetRect(rect, ASTextNSEdgeInsetsInvert(container.insets));
      }
      rect = CGRectStandardize(rect);
      ...
      if (container.truncationType != ASTextTruncationTypeNone) {
        ...
        if (container.truncationToken) {
          ...
          if (runCount > 0) {
            ...
            NSFont *uiFont = [NSFont systemFontOfSize:fontSize * 0.9];
            if (uiFont) {
...
- (ASTextPosition *)positionForPoint:(CGPoint)point
  ...
  if (_container.isVerticalForm) {
    if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
      ASTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:NSTextLayoutDirectionUp offset:1];
      if (range) return range.start;
    } else { // search forward
      ASTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:NSTextLayoutDirectionDown offset:1];
      if (range) return range.end;
    }
  } else {
    if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
      ASTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:NSTextLayoutDirectionLeft offset:1];
      if (range) return range.start;
    } else { // search forward
      ASTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:NSTextLayoutDirectionRight offset:1];
      if (range) return range.end;
    }
  }
...
- (ASTextRange *)textRangeAtPoint:(CGPoint)point {
  ...
  if (_container.verticalForm) {
    ASTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? NSTextLayoutDirectionUp:NSTextLayoutDirectionDown offset:1];
    return range;
  } else {
    ASTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? NSTextLayoutDirectionLeft:NSTextLayoutDirectionRight offset:1];
    return range;
  }
}
...
- (ASTextRange *)closestTextRangeAtPoint:(CGPoint)point {
  ...
  NSTextLayoutDirection direction = NSTextLayoutDirectionRight;
  if (pos.offset >= line.range.location + line.range.length) {
    if (direction != RTL) {
      direction = _container.verticalForm ? NSTextLayoutDirectionUp : NSTextLayoutDirectionLeft;
    } else {
      direction = _container.verticalForm ? NSTextLayoutDirectionDown : NSTextLayoutDirectionRight;
    }
    ...
    if (_container.verticalForm) {
      direction = (rect.origin.y >= point.y && !RTL) ? NSTextLayoutDirectionUp:NSTextLayoutDirectionDown;
    } else {
      direction = (rect.origin.x >= point.x && !RTL) ? NSTextLayoutDirectionLeft:NSTextLayoutDirectionRight;
    }
  }
...
- (ASTextRange *)textRangeByExtendingPosition:(ASTextPosition *)position
                                  inDirection:(NSTextLayoutDirection)direction
  ...
  if (isVerticalForm) {
    verticalMove = direction == NSTextLayoutDirectionLeft || direction == NSTextLayoutDirectionRight;
    forwardMove = direction == NSTextLayoutDirectionLeft || direction == NSTextLayoutDirectionDown;
  } else {
    verticalMove = direction == NSTextLayoutDirectionUp || direction == NSTextLayoutDirectionDown;
    forwardMove = direction == NSTextLayoutDirectionDown || direction == NSTextLayoutDirectionRight;
  }
  ...
  if (!isVertical && !glyphTransformValue) { // draw run
    ...
    CGContextSaveGState(context); {
      ...
      if (isVertical) {
        ...
        CGAffineTransform glyphTransform = glyphTransformValue.as_CGAffineTransformValue;
...
static void ASTextDrawBorderRects(CGContextRef context, CGSize size, ASTextBorder *border, NSArray *rects, BOOL isVertical) {
  ...
  for (NSValue *value in rects) {
    ...
    if (isVertical) {
      rect = NSEdgeInsetsInsetRect(rect, UIEdgeInsetRotateVertical(border.insets));
    } else {
      rect = NSEdgeInsetsInsetRect(rect, border.insets);
    }
...
static void ASTextDrawBlockBorder(ASTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
  ...
  for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
    ...
    for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      if (isVertical) {
        NSEdgeInsets insets = layout.container.insets;
      ...
      ASTextDrawBorderRects(context, size, border, @[[NSValue valueWithRect:unionRect]], isVertical);
  ...
  for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
    ...
    for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      for (NSInteger ll = l; ll < lMax; ll++) {
        ...
        if (!CGRectIsNull(extLineRect)) {
          [runRects addObject:[NSValue valueWithRect:extLineRect]];
        }
      }
      
      NSMutableArray *drawRects = [NSMutableArray new];
      CGRect curRect= ((NSValue *)[runRects firstObject]).rectValue;
      for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
        CGRect rect = ((NSValue *)runRects[re]).rectValue;
        if (isVertical) {
          if (fabs(rect.origin.x - curRect.origin.x) < 1) {
            curRect = ASTextMergeRectInSameLine(rect, curRect, isVertical);
          } else {
            [drawRects addObject:[NSValue valueWithRect:curRect]];
            curRect = rect;
          }
        } else {
...
static void ASTextDrawAttachment(ASTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, NSView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
  ...
  for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
    ...
    NSView *view = nil;
    CALayer *layer = nil;
    if ([a.content isKindOfClass:[NSImage class]]) {
      image = a.content;
    } else if ([a.content isKindOfClass:[NSView class]]) {
      view = a.content;
    ...
    CGRect rect = ((NSValue *)layout.attachmentRects[i]).rectValue;
    if (isVertical) {
      rect = NSEdgeInsetsInsetRect(rect, UIEdgeInsetRotateVertical(a.contentInsets));
    } else {
      rect = NSEdgeInsetsInsetRect(rect, a.contentInsets);
    }
    ...
    if (image) {
      CGImageRef ref = CGImageFromNSImage(image);
      if (ref) {
        CGContextSaveGState(context);
...
CGImageRef CGImageFromNSImage(NSImage *image) {
    ...
    // Attempt to get the best representation of the image
    ...
    // If there's no existing NSBitmapImageRep, create one
    if (!bitmapRep) {
        [image lockFocus];
    ...
    // Ensure we have a valid CGImageRef
    ...
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      while (shadow) {
        ...
        CGContextSaveGState(context); {
          ...
          CGContextBeginTransparencyLayer(context, NULL); {
            NSColor *opaqueShadowColor = [shadow.color colorWithAlphaComponent:1];
...
static void ASTextDrawDebug(ASTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, ASTextDebugOption *op) {
  // Начало контекста рисования
      ...
      if (op.CTFrameBorderColor || op.CTFrameFillColor) {
          NSBezierPath *path = layout.container.path;
          if (!path) {
              NSRect rect = NSMakeRect(0, 0, layout.container.size.width, layout.container.size.height);
              NSEdgeInsets insets = layout.container.insets;
              rect = NSEdgeInsetsInsetRect(rect, insets);
          ...
          for (NSBezierPath *ex in layout.container.exclusionPaths) {
              [path appendBezierPath:ex];
          }
      ...
      for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
          ...
          if (op.CTLineNumberColor) {
              ...
              NSMutableAttributedString *num = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%lu", (unsigned long)l]];
              [num addAttribute:NSForegroundColorAttributeName value:op.CTLineNumberColor range:NSMakeRange(0, num.length)];
              [num addAttribute:NSFontAttributeName value:[NSFont systemFontOfSize:6] range:NSMakeRange(0, num.length)];
              [num drawAtPoint:NSMakePoint(line.position.x, line.position.y - (isVertical ? 1 : 6))];
          }
          if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
              ...
              for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                  ...
                  if (op.CTRunNumberColor) {
                      ...
                      NSMutableAttributedString *num = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%lu", (unsigned long)r]];
                      [num addAttribute:NSForegroundColorAttributeName value:op.CTRunNumberColor range:NSMakeRange(0, num.length)];
                      [num addAttribute:NSFontAttributeName value:[NSFont systemFontOfSize:6] range:NSMakeRange(0, num.length)];
                      [num drawAtPoint:NSMakePoint(runTypoBounds.origin.x, runTypoBounds.origin.y - 1)];
                  }
