...
typedef NS_ENUM(NSInteger, NSTextLayoutDirection) {
    NSTextLayoutDirectionRight,
    NSTextLayoutDirectionLeft,
    NSTextLayoutDirectionUp,
    NSTextLayoutDirectionDown
};
...
+ (instancetype)containerWithPath:(nullable NSBezierPath *)path NS_RETURNS_RETAINED;
...
/// The insets for constrained size. The inset value should not be negative. Default is NSEdgeInsetsZero.
@property NSEdgeInsets insets;

/// Custom constrained path. Set this property to ignore `size` and `insets`. Default is nil.
@property (nullable, copy) NSBezierPath *path;

/// An array of `UIBezierPath` for path exclusion. Default is nil.
@property (nullable, copy) NSArray<NSBezierPath *> *exclusionPaths;
...
///< Set of Attachment (NSImage/UIView/CALayer)
