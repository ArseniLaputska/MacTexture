...
{
  _defaultControlsColor = [NSColor whiteColor];
  ...
  if (_scrubberNode == nil) {
    ...
    _scrubberNode = [[ASDisplayNode alloc] initWithViewBlock:^UIView * _Nonnull {
      ...
      if (strongSelf->_delegateFlags.delegateScrubberThumbImage) {
        NSImage *thumbImage = [strongSelf.delegate videoPlayerNodeScrubberThumbImage:strongSelf];
...
- (void)showSpinner
{
  ...
  if (!_spinnerNode) {
    ...
    _spinnerNode = [[ASDisplayNode alloc] initWithViewBlock:^UIView *{
      ...
      spinnnerView.backgroundColor = [NSColor clearColor];
...
- (ASLayoutSpec *)defaultLayoutSpecThatFits:(CGSize)maxSize
{
  ...
  NSEdgeInsets insets = NSEdgeInsetsMake(10.0, 10.0, 10.0, 10.0);
