...
- (void)generatePlaceholderImage
{
  ...
  [self imageAtTime:kCMTimeZero completionHandler:^(NSImage *image) {
...
- (void)imageAtTime:(CMTime)imageTime completionHandler:(void(^)(NSImage *image))completionHandler
{
  ASPerformBlockOnBackgroundThread(^{
    ...
    [previewImageGenerator generateCGImagesAsynchronouslyForTimes:@[[NSValue valueWithCMTime:imageTime]]
                                                completionHandler:^(CMTime requestedTime, CGImageRef image, CMTime actualTime, AVAssetImageGeneratorResult result, NSError *error) {
                                                  ...
                                                  completionHandler(image ? [[NSImage alloc] initWithCGImage:image size:NSZeroSize] : nil);
                                                }];
  });
}

- (void)setVideoPlaceholderImage:(NSImage *)image
{
