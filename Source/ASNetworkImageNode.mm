...
- (void)_locked__setImage:(NSImage *)image
{
  DISABLED_ASAssertLocked(__instanceLock__);
...
- (void)_locked_setDefaultImage:(NSImage *)defaultImage
{
...
- (void)displayWillStartAsynchronously:(BOOL)asynchronously
{
  ...
  if (asynchronously == NO && _networkImageNodeFlags.cacheSupportsSynchronousFetch) {
    ...
    if (_networkImageNodeFlags.imageLoaded == NO && url && _downloadIdentifier == nil) {
      NSImage *result = [[_cache synchronouslyFetchedCachedImageWithURL:url] asdk_image];
      if (result) {
        [self _setCurrentImageQuality:1.0];
        [self _setDownloadProgress:1.0];
...
- (void)handleProgressImage:(NSImage *)progressImage progress:(CGFloat)progress downloadIdentifier:(nullable id)downloadIdentifier
{
  ASLockScopeSelf();
...
- (void)_updateProgressImageBlockOnDownloaderIfNeeded
{
  ...
  if (newDownloadIDForProgressBlock != nil) {
    ...
    [_downloader setProgressImageBlock:^(NSImage * _Nonnull progressImage, CGFloat progress, id  _Nullable downloadIdentifier) {
    ...
    if (URL.isFileURL) {
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self->_networkImageNodeFlags.shouldCacheImage) {
          [self _locked__setImage:[NSImage imageNamed:URL.path.lastPathComponent]];
        } else {
          // First try to load the path directly, for efficiency assuming a developer who
          // doesn't want caching is trying to be as minimal as possible.
          auto nonAnimatedImage = [[NSImage alloc] initWithContentsOfFile:URL.path];
          if (nonAnimatedImage == nil) {
            ...
            if (filename != nil) {
              nonAnimatedImage = [[NSImage alloc] initWithContentsOfFile:filename];
