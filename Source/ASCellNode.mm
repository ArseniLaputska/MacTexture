...
- (instancetype)init
{
  ...
  _selectionStyle = NSTableViewSelectionHighlightStyleNone;
//  _focusStyle = NSTableViewCellFocusStyleDefault;
...
- (void)didLoad
{
  ...
  if (_viewControllerBlock != nil) {
    ...
    if ([_viewController isKindOfClass:[ASDKViewController class]]) {
      ...
      NSViewController *viewController = _viewController;
...
- (void)cellNodeVisibilityEvent:(ASCellNodeVisibilityEvent)event inScrollView:(NSScrollView *)scrollView withCellFrame:(CGRect)cellFrame
{
  // To be overriden by subclasses
}
...
- (void)handleVisibilityChange:(BOOL)isVisible
{
  ...
  NSScrollView *scrollView = (_scrollView != nil && view.superview != nil && [view isDescendantOf:_scrollView]) ? _scrollView : nil;
  if (scrollView) {
...
- (instancetype)initWithAttributes:(NSDictionary *)textAttributes insets:(NSEdgeInsets)textInsets
{
  self = [super init];
  if (self) {
...
{
  return @{NSFontAttributeName : [NSFont systemFontOfSize:kASTextCellNodeDefaultFontSize]};
}

+ (NSEdgeInsets)defaultTextInsets
{
    return NSEdgeInsetsMake(kASTextCellNodeDefaultVerticalPadding, kASTextCellNodeDefaultHorizontalPadding, kASTextCellNodeDefaultVerticalPadding, kASTextCellNodeDefaultHorizontalPadding);
}
...
{
  if (ASLockedSelfCompareAssignCustom(_textInsets, textInsets, NSEdgeInsetsEqualToEdgeInsets)) {
    [self setNeedsLayout];
  }
}
