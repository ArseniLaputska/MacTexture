...
{
  NSUInteger viewControllerIndex = [self.childViewControllers indexOfObjectIdenticalTo:childViewController];
  ...
  if (viewControllerIndex == self.childViewControllers.count - 1) {
  ...
  return [self visibilityDepth] + self.childViewControllers.count - 1 - viewControllerIndex;
}
...
- (void)setViewControllers:(NSArray *)viewControllers
{
  ...
  [super setChildViewControllers:viewControllers];
  [self visibilityDepthDidChange];
}
...
//  NSViewController *viewController = [super popViewControllerAnimated:animated];
