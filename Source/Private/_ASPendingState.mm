...
#define __shouldSetNeedsDisplayForLayer(layer) (flags.needsDisplay \
  ...
  || (flags.setBackgroundColor && ![backgroundColor isEqual:[NSColor colorWithCGColor:(layer).backgroundColor]]))
...
- (instancetype)init
{
  ...
  layoutMargins = NSEdgeInsetsMake(8, 8, 8, 8);
...
{
  ASDisplayNodeAssert(!isnan(newBounds.size.width) && !isnan(newBounds.size.height), @"Invalid bounds %@ provided to %@", NSStringFromRect(newBounds), self);
...
{
  ASDisplayNodeAssert(!isnan(newPosition.x) && !isnan(newPosition.y), @"Invalid position %@ provided to %@", NSStringFromPoint(newPosition), self);
...
- (void)setLayoutMargins:(NSEdgeInsets)margins
{
...
- (void)setAccessibilityPath:(NSBezierPath *)newAccessibilityPath
{
...
- (void)applyToView:(NSView *)view withSpecialPropertiesHandling:(BOOL)specialPropertiesHandling
{
  /*
...
+ (_ASPendingState *)pendingViewStateFromLayer:(CALayer *)layer
{
  ...
  pendingState.backgroundColor = [NSColor colorWithCGColor:layer.backgroundColor];
...
// FIXME: Make this more efficient by tracking which properties are set rather than reading everything.
+ (_ASPendingState *)pendingViewStateFromView:(NSView *)view
{
  if (!view) {
    return nil;
  }
