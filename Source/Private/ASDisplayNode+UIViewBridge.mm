...
{
  ASDisplayNodeAssertMainThread();
  return !self.layerBacked && [self.view performSelector:action withObject:sender];
}

- (CGFloat)alpha
{
  _bridge_prologue_read;
//  return _getFromViewOnly(opacity, alphaValue);
  return self.alpha;
}
    ...
    if (!specialPropertiesHandling) {
      ...
      if (canReadProperties) {
        ...
        if (ASIsCGRectValidForLayout(newBounds) == NO || ASIsCGPositionValidForLayout(newPosition) == NO) {
          ASDisplayNodeAssertNonFatal(NO, @"-[ASDisplayNode setFrame:] - The new frame (%@) is invalid and unsafe to be set.", NSStringFromRect(rect));
          return;
        }
...
- (void)setOpaque:(BOOL)newOpaque
{
  ...
  if (shouldApply) {
    /*
     ...
     - Set the backgroundColor of the node to a valid NSColor
...
- (BOOL)isUserInteractionEnabled
{
  ...
  return true/*_getFromViewOnly(userInteractionEnabled)*/;
}
...
- (void)setBackgroundColor:(NSColor *)newBackgroundColor
{
  ...
  BOOL shouldApply = ASDisplayNodeShouldApplyBridgedWriteToView(self);
  if (shouldApply) {
    NSColor *oldBackgroundColor = _backgroundColor;
    ...
    if (_flags.layerBacked) {
      ...
      /*
       ...
       Given that NSColor / UIView has dynamic capabilties now, we should set directly to the view and make sure that the layers value is consistent here.

       */
//      _view.layer.backgroundColor = _backgroundColor;
      ...
      _layer.backgroundColor = _view.layer.backgroundColor;
    }
...
- (NSEdgeInsets)layoutMargins
{
  _bridge_prologue_read;
  ASDisplayNodeAssert(!_flags.layerBacked, @"Danger: this property is undefined on layer-backed nodes.");
//  NSEdgeInsets margins = _getFromViewOnly(layoutMargins);

  return NSEdgeInsetsZero;
}

- (void)setLayoutMargins:(NSEdgeInsets)layoutMargins
{
  _bridge_prologue_write;
  ASDisplayNodeAssert(!_flags.layerBacked, @"Danger: this property is undefined on layer-backed nodes.");
...
//  return _getAccessibilityFromViewOrProperty(_accessibilityAttributedHint, accessibilityAttributedHint);
  return self.accessibilityAttributedHint;
}
...
//  return _getAccessibilityFromViewOrProperty(_accessibilityAttributedValue, accessibilityAttributedValue);
  return self.accessibilityAttributedValue;
}
...
//  return _getAccessibilityFromViewOrProperty(_accessibilityLanguage, accessibilityLanguage);
  return self.accessibilityLanguage;
}
...
//  return _getAccessibilityFromViewOrProperty(_flags.accessibilityElementsHidden, accessibilityElementsHidden);
  return self.accessibilityElementsHidden;
}
...
//  return _getAccessibilityFromViewOrProperty(_flags.accessibilityViewIsModal, accessibilityViewIsModal);
  return self.accessibilityViewIsModal;
}
...
//  return _getAccessibilityFromViewOrProperty(_flags.shouldGroupAccessibilityChildren, shouldGroupAccessibilityChildren);
  return self.shouldGroupAccessibilityChildren;
}
...
//  return _getAccessibilityFromViewOrProperty(_accessibilityCustomActions, accessibilityCustomActions);
  return self.accessibilityCustomActions;
}
