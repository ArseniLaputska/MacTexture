...
@package
  ...
  NSEdgeInsets _textContainerInsets;
...
- (instancetype)initWithRendererAttributes:(ASTextKitAttributes)rendererAttributes
                       ...
                       textContainerInsets:(NSEdgeInsets)textContainerInsets
...
{
  CGRect rect = NSEdgeInsetsInsetRect(bounds, _textContainerInsets);
...
@interface ASTextNode () <NSGestureRecognizerDelegate>
...
@implementation ASTextNode {
  ...
  NSColor *_cachedShadowNSColor;
  NSColor *_cachedTintColor;
  NSColor *_placeholderColor;
  CGFloat _shadowOpacity;
  CGFloat _shadowRadius;
  
  NSEdgeInsets _textContainerInset;
  ...
  NSPressGestureRecognizer *_longPressGestureRecognizer;
...
//    self.accessibilityTraits = self.defaultAccessibilityTraits;
    ...
    _placeholderInsets = NSEdgeInsetsMake(1.0, 0.0, 1.0, 0.0);
...
{
  DISABLED_ASAssertLocked(__instanceLock__);
  bounds = NSEdgeInsetsInsetRect(bounds, _textContainerInset);
...
- (ASTextKitAttributes)_locked_rendererAttributes
{
  ...
  return {
    ...
    .shadowColor = _cachedShadowNSColor,
...
- (NSAccessibilityRole)defaultAccessibilityTraits
{
...
{
  if (ASLockedSelfCompareAssignCustom(_textContainerInset, textContainerInset, NSEdgeInsetsEqualToEdgeInsets)) {
    [self setNeedsLayout];
  }
}
...
- (NSEdgeInsets)textContainerInset
{
...
+ (CGFloat)ascenderWithAttributedString:(NSAttributedString *)attributedString 
{
  ...
  CGFloat lineHeight = MAX(0, paragraphStyle.minimumLineHeight);
...
+ (NSImage *)displayWithParameters:(id<NSObject>)parameters isCancelled:(NS_NOESCAPE asdisplaynode_iscancelled_block_t)isCancelled
{
  ...
  NSEdgeInsets textContainerInsets = drawParameter ? drawParameter->_textContainerInsets : NSEdgeInsetsZero;
  ...
  NSImage *result = ASGraphicsCreateImage(drawParameter->_traitCollection,
                                          CGSizeMake(drawParameter->_bounds.size.width, drawParameter->_bounds.size.height),
                                          drawParameter->_opaque,
                                          drawParameter->_contentScale, nil, nil, ^{
      ...
      if (backgroundColor != nil) {
          ...
          NSRectFillUsingOperation(NSRectFromCGRect(CGContextGetClipBoundingBox(context)), NSCompositingOperationCopy);
      }
...
- (BOOL)gestureRecognizerShouldBegin:(NSGestureRecognizer *)gestureRecognizer
{
  ...
  if (([self _pendingLinkTap] || [self _pendingTruncationTap])
      && [gestureRecognizer isKindOfClass:[NSClickGestureRecognizer class]]
    ...
    if (0 != highlightRange.length) {
      ...
      if (highlightTargetLayer != nil) {
        ...
        for (NSValue *rectValue in highlightRects) {
          NSEdgeInsets shadowPadding = renderer.shadower.shadowPadding;
          CGRect rendererRect = ASTextNodeAdjustRenderRectForShadowPadding(rectValue.rectValue, shadowPadding);
          ...
          [converted addObject:[NSValue valueWithRect:highlightedRect]];
        }
...
{
  return [NSColor colorWithWhite:(style == ASTextNodeHighlightStyleLight ? 0.0 : 1.0) alpha:1.0].CGColor;
}
...
static CGRect ASTextNodeAdjustRenderRectForShadowPadding(CGRect rendererRect, NSEdgeInsets shadowPadding) {
...
- (NSArray *)_rectsForTextRange:(NSRange)textRange measureOption:(ASTextKitRendererMeasureOption)measureOption
{
  ...
  for (NSValue *rectValue in rects) {
    ...
    NSValue *adjustedRectValue = [NSValue valueWithRect:rect];
...
- (NSImage *)placeholderImage
{
  ...
  // Создаем NSImage с указанными размерами
...
- (void)_handleLongPress:(NSPressGestureRecognizer *)longPressRecognizer
{
  ASDisplayNodeAssertMainThread();
...
- (void)setShadowColor:(CGColorRef)shadowColor
{
  ...
  if (_shadowColor != shadowColor && CGColorEqualToColor(shadowColor, _shadowColor) == NO) {
    ...
    _cachedShadowNSColor = [NSColor colorWithCGColor:shadowColor];
    [self unlock];
